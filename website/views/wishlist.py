...
class WishlistView(View):
    def post(self, request):
        ...
        if wish_list_type == 'add':
            product = Product.objects.get(id=get_request('product_id'))
            ...
            UserWihslistProduct.objects.get_or_create(wishlist=wishlist, product=product)

            if get_request('remove_wishlist'):
                default_wish_list, created = UserWishlist.objects.get_or_create(
                    user_id=user.id, name='wish list')
                print(default_wish_list.id)
                UserWihslistProduct.objects.get(wishlist=default_wish_list, product=product).delete()
